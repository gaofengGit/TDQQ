...
namespace TDQQ.Common
{
    ...
    {
        /// <summary>
        /// 填充错误信息至LogFile.txt文件中
        /// </summary>
        /// <param name="filePath"></param>
        /// <param name="toWriteInfo"></param>
        public static void WriteErrorInfo(string filePath, string toWriteInfo)
        {
            ...
            using (var fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Write))
            {
                ...
                streamWriter.Write(toWriteInfo);
                streamWriter.Flush();
                fileStream.Close();
            }
        }

        public static int Col(char col)
        {
            const int errrorIndex = -1;
            if (col > 'Z' || col < 'A')
            {
                return errrorIndex;
            }
        ...
        /// <returns></returns>
        public static Boolean IsDisposed(System.Windows.Window window)
        {
            return new WindowInteropHelper(window).Handle == IntPtr.Zero;
        }

        #region 将数字表达为传统的大写表示
